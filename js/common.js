...
function initLiveRates() {
    ...
    const goldEl = document.getElementById('rateGold');
    ...
    function parseAndUpdate(raw) {
        ...
        items.forEach(item => {
            ...
            if (id === '1' && goldEl) {
                goldEl.textContent = '₹ ' + formatPrice(buyPrice) + '/gm';
            } else if (id === '3' && silverEl) {
                silverEl.textContent = '₹ ' + formatPrice(buyPrice) + '/gm';
            }
        });
    }
